...
[RAG](https://en.wikipedia.org/wiki/Retrieval-augmented_generation) is essential. It's also hard to get right. Production RAG systems often disappoint, and the response is typically piling on workarounds: [HyDE](https://docs.haystack.deepset.ai/docs/hypothetical-document-embeddings-hyde), reranking, query expansion, chunk overlap tuning. Complexity grows; results don't necessarily improve.

The problem isn't the workarounds. It's what they're working around.

The dominant RAG frameworks--most notably, LangChain—-are built on an obsolete model that dates to 2023, before tool calling changed everything. They use static pipelines when what's needed is fundamentally different: **letting the LLM reason about retrieval rather than blindly executing a predetermined flow**.

The research is clear. The future of RAG is agentic.
...
The `ContentRetriever` is configured once, and supplements the prompt to help answer queries. The [documentation explicitly describes](https://docs.langchain.com/oss/python/langchain/rag) this as a "two-step chain" that provides "reduced latency at the expense of flexibility."
...
You can build more elaborate pipelines with old school RAG frameworks, but can't  fix the core problem with this approach.
...
Recently, more and more papers and articles are calling out these limitations.
...
Embabel was built from the ground up for agentic AI. Our new RAG support sits cleanly on top of the core framework through the `LlmReference` abstraction.
